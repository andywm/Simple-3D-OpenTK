...
namespace Labs.ACW
{
    ...
    public class Ball : Sphere //Is Physics Object...
    {
        ...
        public void KillRegions()
        {
            foreach (SpatialRegion r in CollisionCheckRegion)
            {
                r.BallsInRegion.Remove(this);
            }
        }
        ...
        {
            if (!World.gamePhysics.showPhysicsData) return;
    ...
    public class Surface : Shape //Probably Obsolete.
    {
        ...
        public Surface(ref Vector3 v3, ref float mySize, ref Aesthetics.Material mater, bool flip)
        {
            SetPosition(ref v3);
            SetCubeSize(ref mySize);
            InitTransform(flip);
        ...
        protected void InitTransform(bool flip)
        {
            mTransformations = Matrix4.CreateScale(size * ACWWindow.Centimetre, size * ACWWindow.Centimetre, size * ACWWindow.Centimetre) * Matrix4.CreateRotationX((float)Math.PI) * Matrix4.CreateTranslation(vPosition.X, vPosition.Y, vPosition.X);
        }
        #endregion
